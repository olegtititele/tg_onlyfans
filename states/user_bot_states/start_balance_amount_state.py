...
async def message_handler(message: types.Message, chat_id):
    ...
    try:
        ...
        if sum >= 1 and sum <= 1000:
            db.update_bot_referal_sum(current_bot, sum)
            
            await bot.send_photo(
                chat_id=chat_id, 
                caption=f"<b>Стартовый баланс изменен на {sum} ₽.</b>",
                photo=photo,
            ...
            await bot.send_photo(
                chat_id=chat_id, 
                caption=f"<b>Стартовый баланс должен быть больше 1 и меньше 1000. Попробуйте еще раз.</b>",
                photo=photo,
                parse_mode=ParseMode.HTML,
                reply_markup=kb.back_to_user_bot_info_kb()
            )
        ...
        await bot.send_photo(
            chat_id=chat_id, 
            caption=f"<b>Стартовый баланс должнен быть числом. Попробуйте еще раз.</b>",
            photo=photo,
            parse_mode=ParseMode.HTML,
            reply_markup=kb.back_to_user_bot_info_kb()
        )